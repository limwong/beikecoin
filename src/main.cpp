...
#if defined(NDEBUG)
# error "BeiKecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7facd80874a088d0a3ad50d25b12e72b064e9086c35e8212b6cdc754168ccd4a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BeiKecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BeiKecoin Signed Message:\n";
...
{
    // BeiKecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // BeiKecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // BeiKecoin: 3.5 days
static const int64 nTargetSpacing = 1 * 30; // BeiKecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BeiKecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BeiKecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7facd80874a088d0a3ad50d25b12e72b064e9086c35e8212b6cdc754168ccd4a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("0x211fe0a0da279dbdec0652217a02b93eddd9f994af403fe063ef1e6a4b459653"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // BeiKecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BeiKecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BeiKecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BeiKecoinMiner(CWallet *pwallet)
{
    printf("BeiKecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BeiKecoin-miner");
    ...
    try { loop {
        ...
        printf("Running BeiKecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BeiKecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BeiKecoinMiner, pwallet));
}
